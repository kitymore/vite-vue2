...
git clone https://github.com/kitymore/vite-vue2.git
...
&emsp;&emsp;以 `VITE_APP_` 开头的变量，在代码中可以通过 `process.env.VITE_APP_` 访问。  
&emsp;&emsp;比如,`VITE_APP_ENV = 'development'` 通过`process.env.VITE_APP_ENV` 访问。  
&emsp;&emsp;除了 `VITE_APP_*` 变量之外，在你的应用代码中始终可用的还有两个特殊的变量`NODE_ENV` 和`BASE_URL`
